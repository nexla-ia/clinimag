...
export default function CompanyConversations() {
  ...
  async function handlePaste(e) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const t = file.type || ''
      // Só mídia suportada (imagem/pdf/vídeo) — igual ao anexo pelo clipe.
      if (!(t.startsWith('image/') || t === 'application/pdf' || t.startsWith('video/'))) continue
      e.preventDefault() // não deixa o navegador colar nada no texto
      // Print vem sem nome → dá um nome amigável com a extensão certa.
        ...
        <div style={{
          ...
          <div className="nx-card" style={{ width: '100%', maxWidth: 440, maxHeight: 'calc(100vh - 3rem)', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '1rem 1.5rem', flex: 1, overflowY: 'auto', minHeight: 0 }}>
              {(() => {
                // Transfere pra qualquer usuário cadastrado na empresa (inclusive admin),
                // só não pra você mesmo.
